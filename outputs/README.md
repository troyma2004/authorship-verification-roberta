...
Both contain one `prediction` column with binary values (`1`: same author; `0`: different authors), retained in their historical row order. The original checkpoints and the unlabelled coursework test input have since been recovered. That input contains 5,985 rows, matching the coursework prediction count, but input-to-output correspondence has not been recomputed through inference. No labels or independent test scores are supplied.
