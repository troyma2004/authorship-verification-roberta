...
The repository preserves the training and inference implementations, recorded development results, and the four recovered trained checkpoints. The checkpoints are distributed separately through a [GitHub release](https://github.com/troyma2004/authorship-verification-roberta/releases/tag/checkpoints-2026).
...
To prepare the inference notebook in Google Colab:

1. Download [`models.zip`](https://github.com/troyma2004/authorship-verification-roberta/releases/download/checkpoints-2026/models.zip), `model_manifest.json`, and `SHA256SUMS.txt` from the [release](https://github.com/troyma2004/authorship-verification-roberta/releases/tag/checkpoints-2026). With all three files in one directory, verify them using `shasum -a 256 -c SHA256SUMS.txt` (macOS) or `sha256sum -c SHA256SUMS.txt` (Linux), then upload the ZIP to `/content/models.zip`.
...
| `models/` | Model metadata, configurations, tokenizers, and checkpoint manifest; parameter files are in the release download |
